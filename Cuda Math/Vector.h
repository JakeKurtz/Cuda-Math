...
namespace jek
{
	...
	template <class T> _HOST_DEVICE 
	Vec4<T> operator*(const T s, const Vec4<T>& v)
	{
		return Vec4<T>(s * v.x, s * v.y, s * v.z, s * v.w);
	};
	template <class T> _HOST_DEVICE 
	Vec4<T> operator*(const Vec4<T>& v, const T s)
	{
		return Vec4<T>(s * v.x, s * v.y, s * v.z, s * v.w);
	};
	...
	template<class T> _HOST_DEVICE
	inline Vec3<T> reflect(const Vec3<T>& i, const Vec3<T>& r)
	{
	...
	static inline _HOST_DEVICE
	Vec2f sample_spherical_map(const Vec4f& d)
	{
		Vec2f uv = Vec2f(0.5f + ::atan2(d.z, d.x) * M_1_2PI, 0.5f - ::asin(d.y) * M_1_PI);
		return uv;
	}
	static inline _HOST_DEVICE
	Vec4f sample_spherical_direction(const Vec2f& uv)
	{
